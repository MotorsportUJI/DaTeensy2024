...
namespace OBD2
{
    ...
    typedef struct
    {
        ...
        float distance_traveled_MIL_on_A = 0;
        float distance_traveled_MIL_on_B = 0; // formula : 256A + B

        float absolute_barometric_presure = 0; // formula : A

        float control_module_voltage_A = 0;
        float control_module_voltage_B = 0; // formula : (256A + B) / 1000

        float relavite_throttle_position = 0; // formula : A/2.55

        float time_run_with_mil_on_A = 0;
        float time_run_with_mil_on_B = 0; // formula : 256A + B minutes
    ...
    float getMIL();                       // MIL on
    float getDTC();                       // DTC count
    float getFuelSys();                   // fuel system status
    float getEngineLoad();                // engine load
    float getEngineCoolantTemp();         // engine coolant temp
    float getLongTermFuelTrim();          // long term fuel trim
    float getIntakeManifoldAbsPressure(); // intake manifold absolute pressure
    float getRPM();                       // engine rpm
    float getSpeed();                     // vehicle speed
    float getTimingAdvance();             // timing advance
    float getAirIntakeTemp();             // air intake temp
    float getThrottlePosition();          // throttle position
    float getOxygenSensor1();             // oxygen sensor 1
    float getDistanceTraveledMIL();       // distance traveled with MIL on
    float getBarometricPressure();        // absolute barometric pressure
    float getControlModuleVoltage();      // control module voltage
    float getRelativeThrottlePosition();  // relative throttle position
    float getTimeRunWithMIL();            // time run with MIL on
    float getOxygenSensorVoltage();       // oxygen sensor voltage
    float getOxygenSensorFuelTrim();      // oxygen sensor fuel trim
    float getObdTPS();                    // throttle position sensor
